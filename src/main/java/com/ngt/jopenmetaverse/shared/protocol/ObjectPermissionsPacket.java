...
import com.ngt.jopenmetaverse.shared.util.Utils;
	public final class ObjectPermissionsPacket extends Packet
    {
		...
		public byte Set;
			...
			public void FromBytes(byte[] bytes, int[] i) throws MalformedDataException
            {
                try
                {
                    ...
                    Set = (byte)bytes[i[0]++];
                    Mask = Utils.bytesToUIntLit(bytes, i[0]); i[0] += 4;
                }
                catch (Exception e)
                {
                    throw new MalformedDataException(Utils.getExceptionStackTraceAsString(e));
			...
			public void ToBytes(byte[] bytes, int[] i)
            {
                ...
                bytes[i[0]++] = Field;
                bytes[i[0]++] = Set;
                Utils.uintToBytesLit(Mask, bytes, i[0]); i[0] += 4;
            }
        ...
        public ObjectPermissionsPacket(byte[] bytes, int[] i) throws MalformedDataException 
		{
		this();
        ...
        public ObjectPermissionsPacket(Header head, byte[] bytes, int[] i) throws MalformedDataException
		{
		this();
            int[] packetEnd = new int[] {bytes.length - 1};
