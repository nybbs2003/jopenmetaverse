    ...
    public final class TestMessagePacket extends Packet
    {
        ...
        public final class TestBlock1Block extends PacketBlock
        {
            ...
            public void FromBytes(byte[] bytes, int i[]) throws MalformedDataException
            {
                try
                {
                    test1 = (long)(bytes[i[0]++] + (bytes[i[0]++] << 8) + (bytes[i[0]++] << 16) + (bytes[i[0]++] << 24));
                }
                catch (Exception e)
                {
        ...
        public final class NeighborBlockBlock extends PacketBlock
        {
            ...
            public void FromBytes(byte[] bytes, int i[]) throws MalformedDataException
            {
                try
                {
                    test0 = (long)(bytes[i[0]++] + (bytes[i[0]++] << 8) + (bytes[i[0]++] << 16) + (bytes[i[0]++] << 24));
                    test1 = (long)(bytes[i[0]++] + (bytes[i[0]++] << 8) + (bytes[i[0]++] << 16) + (bytes[i[0]++] << 24));
                    test2 = (long)(bytes[i[0]++] + (bytes[i[0]++] << 8) + (bytes[i[0]++] << 16) + (bytes[i[0]++] << 24));
                }
                catch (Exception e)
                {
                    throw new MalformedDataException();
