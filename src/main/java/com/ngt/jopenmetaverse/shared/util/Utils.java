...
{   
	...
	//
	//Number of milliseconds since  12:00 midnight, January 1, 0001 A.D. (C.E.) in the GregorianCalendar calendar 
	...
	private static final String[] _SaleTypeNames = new String[]
			{
		"not",
		"orig",
		"copy",
...
//	}
	...
	public static int bytesToIntLit(byte[] bytes, int pos)
	{
		...
		return ByteBuffer.wrap(bytes).order(ByteOrder.LITTLE_ENDIAN).getInt(pos);
	}
	...
	public static long bytesToInt64Lit(byte[] bytes, int pos)
	{
		...
		//            ((long)bytes[pos + 6] << 48) +
		//            ((long)bytes[pos + 7] << 56));
		return ByteBuffer.wrap(bytes).order(ByteOrder.LITTLE_ENDIAN).getLong(pos);
	}
...
//	        return ((long)bytes[pos + 0] + ((long)bytes[pos + 1] << 8) + ((long)bytes[pos + 2] << 16) + ((long)bytes[pos + 3] << 24));
	...
	public static String bytesToString(byte[] bytes, int index, int count, String charsetName) throws UnsupportedEncodingException
	{
